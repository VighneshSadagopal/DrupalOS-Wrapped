...
import { X, Globe, Zap, GraduationCap, Mic, Sparkles, GitCommit } from 'lucide-react';
...
const StoryPlayer: React.FC<StoryPlayerProps> = ({ data, onExit }) => {
  ...
  const SlideHeader = ({ title, progressIndex }: { title: string | React.ReactNode, progressIndex: number }) => (
    <motion.div 
  ...
  const slides = [
    ...
    // SLIDE 1: IMPACT SUMMARY (MEMBER SINCE)
    {
      id: 'impact-summary',
      render: () => {
        const sinceYear = data.memberSince || 2021;
        const yearsCount = 2025 - sinceYear;
    ...
    // SLIDE 2: TOTAL CONTRIBUTIONS
    {
      ...
      render: () => {
        const total = data.totalContributions || 142;

        return (
          <div className="flex flex-col items-center h-full w-full relative z-10 overflow-hidden bg-[#0a0a0a]">
             <SlideBackground />
             <div className="relative z-10 flex flex-col items-center justify-between h-full w-full py-8 px-6">
               <SlideHeader title="2025 IMPACT" progressIndex={2} />
    ...
    // SLIDE 3: MULTIPLIER (MENTORSHIP)
    {
      id: 'multiplier',
      render: () => {
        const menteeCount = data.menteeCount || 46;

        return (
          <div className="flex flex-col items-center h-full w-full relative z-10 overflow-hidden bg-[#0a0a0a]">
             <SlideBackground />
             <div className="relative z-10 flex flex-col items-center justify-between h-full w-full py-8 px-6">
               <SlideHeader title="YOU’RE A MULTIPLIER" progressIndex={3} />
    ...
    // SLIDE 4: EVENTS (TOOK THE STAGE)
    {
      id: 'events',
      render: () => {
        // Fallback to sample events if empty, just for the visual if no real data is passed
        const events = data.events.length > 0 ? data.events : [
          { title: 'Drupal AI Webinar', id: 'e1' },
          { title: 'DrupalCamp Pune 2025', id: 'e2' }
        ];
        ...
        return (
          <div className="flex flex-col items-center h-full w-full relative z-10 overflow-hidden bg-[#0a0a0a]">
             <SlideBackground />
             <div className="relative z-10 flex flex-col items-center justify-between h-full w-full py-8 px-6">
               <SlideHeader title="YOU TOOK THE STAGE" progressIndex={4} />
               ...
               <div className="flex flex-col items-center text-center gap-8 flex-1 justify-center -mt-10">
                 ...
                 >
                   <div className="w-24 h-24 rounded-full border border-white/80 flex items-center justify-center bg-black/40 backdrop-blur-md z-10 relative">
                     <Mic className="w-10 h-10 text-white stroke-1" />
                   </div>
                   <motion.div 
                 ...
                 <div className="space-y-6 max-w-xs">
                   <motion.h3 
                     ...
                     className="text-2xl md:text-3xl font-extrabold text-white font-display leading-tight uppercase"
                   >
                     IN 2025, YOU ORGANIZED & SPOKE AT MULTIPLE DRUPAL EVENTS
                   </motion.h3>
                   ...
                   >
                     {events.map((evt: any, i) => (
                       <p key={i} className="text-gray-200 text-lg font-medium">
                         {evt.title}
                       </p>
                     ))}
                   </motion.div>
    ...
    // SLIDE 5: DRUPAL CORE (NEW)
    {
      id: 'drupal-core',
      render: () => {
        const coreCount = data.drupalCoreContributionCount || 5;

        return (
          <div className="flex flex-col items-center h-full w-full relative z-10 overflow-hidden bg-[#0a0a0a]">
             <SlideBackground />
             <div className="relative z-10 flex flex-col items-center justify-between h-full w-full py-8 px-6">
               <SlideHeader title="CORE CONTRIBUTOR" progressIndex={5} />
    ...
    // SLIDE 6: AI LEADERSHIP (BROUGHT AI)
    {
      id: 'ai-leadership',
      render: () => {
        const aiCount = data.aiContributionCount || 12;

        return (
          <div className="flex flex-col items-center h-full w-full relative z-10 overflow-hidden bg-[#0a0a0a]">
             <SlideBackground />
             <div className="relative z-10 flex flex-col items-center justify-between h-full w-full py-8 px-6">
               <SlideHeader title="YOU BROUGHT AI TO DRUPAL" progressIndex={6} />
  ...
  const laserVariants = [
    // Intro: Circle
    { top: "30%", left: "50%", x: "-50%", y: "-50%", width: "160px", height: "160px", borderRadius: "50%", borderColor: "#22d3ee", rotate: 0, opacity: 1 },
    // Impact Summary: Large Circle/Lens
    { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "95%", height: "95%", borderRadius: "30px", borderColor: "#3b82f6", rotate: 0, opacity: 0.5 },
    // Total Contributions: Orange/Red
    { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "95%", height: "95%", borderRadius: "30px", borderColor: "#fb923c", rotate: 0, opacity: 0.5 },
    // Multiplier: Green/Blue mix or just Green
    { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "95%", height: "95%", borderRadius: "30px", borderColor: "#4ade80", rotate: 0, opacity: 0.5 },
    // Events: Blue/Purple
    { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "95%", height: "95%", borderRadius: "30px", borderColor: "#60a5fa", rotate: 0, opacity: 0.5 },
    // Drupal Core: Deep Blue/Cyan
    { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "95%", height: "95%", borderRadius: "30px", borderColor: "#0678BE", rotate: 0, opacity: 0.5 },
    // AI Leadership: Purple/Deep Blue
    { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "95%", height: "95%", borderRadius: "30px", borderColor: "#a855f7", rotate: 0, opacity: 0.5 }
  ];
