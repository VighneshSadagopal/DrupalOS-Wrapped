...
import { X, Globe, GraduationCap, Mic, Sparkles, GitCommit, Pause, Play, Linkedin, Twitter, Loader2, Shield, Share2, Calendar, Users, Award, Plus, Code2, Link, Check } from 'lucide-react';
...
const StoryPlayer: React.FC<StoryPlayerProps> = ({ data, onExit }) => {
  ...
  const [avatarSrc, setAvatarSrc] = useState<string>(data.avatarUrl || fallbackAvatar);
  ...
  useEffect(() => {
    const enrichData = async () => {
      try {
        ...
        if (result && result.contributor_roles && Array.isArray(result.contributor_roles) && result.contributor_roles.length > 0) {
  ...
  const SlideBackground = () => (
    <>
      ...
      <motion.div 
        ...
        style={{
          background: 'repeating-conic-gradient(from 0deg at 50% 50%, #333 0deg 2deg, transparent 2deg 15deg)'
        }}
        animate={{ rotate: 360 }}
  ...
  const SlideHeader = ({ title, progressIndex, totalSlides }: { title: string | React.ReactNode, progressIndex: number, totalSlides: number }) => (
    <motion.div 
    ...
    >
      ...
      <div className="w-full flex gap-2 z-20 mb-6">
        {Array.from({ length: totalSlides }).map((_, i) => (
          <div 
            key={i} 
            className={`h-1.5 rounded-full transition-all duration-300 ${i === progressIndex ? 'flex-1 bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]' : 'w-2 bg-white/20'}`} 
          />
        ))}
      </div>
  ...
  const rawSlides: SlideDef[] = [
    // SLIDE 0: INTRO
    {
      id: 'intro',
      condition: true,
      laser: { top: "30%", left: "50%", x: "-50%", y: "-50%", width: "160px", height: "160px", borderRadius: "50%", borderColor: "#22d3ee", rotate: 0, opacity: 1 },
      render: () => (
        <div className="flex flex-col items-center justify-center h-full text-center p-8 relative z-10">
           ...
           >
             <div className="text-cyan-400 font-mono text-sm tracking-[0.2em] uppercase">System Access Granted</div>
    ...
    // SLIDE 1: IMPACT SUMMARY (MEMBER SINCE)
    {
      ...
      laser: { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "95%", height: "95%", borderRadius: "30px", borderColor: "#3b82f6", rotate: 0, opacity: 0.5 },
      render: (index, total) => {
        ...
        return (
          <div className="flex flex-col items-center h-full w-full relative z-10 overflow-hidden bg-[#0a0a0a]">
            ...
            <div className="relative z-10 flex flex-col items-center justify-between h-full w-full py-8 px-6">
              <SlideHeader title={`@${data.username}`} progressIndex={index} totalSlides={total} />
              ...
              <div className="flex flex-col items-center text-center gap-8 flex-1 justify-center -mt-10">
                ...
                >
                  ...
                  <motion.div 
                    animate={{ scale: [1.2, 1.3, 1.2], opacity: [0.5, 0.2, 0.5] }}
                ...
                <div className="space-y-2">
                  ...
                  >
                    That's <span className="text-green-400 font-bold">~{yearsCount} years</span> of<br/>
    ...
    // SLIDE 2: MULTIPLIER (MENTORSHIP)
    {
      ...
      laser: { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "95%", height: "95%", borderRadius: "30px", borderColor: "#4ade80", rotate: 0, opacity: 0.5 },
      render: (index, total) => {
        ...
        return (
          <div className="flex flex-col items-center h-full w-full relative z-10 overflow-hidden bg-[#0a0a0a]">
             <SlideBackground />
             <div className="relative z-10 flex flex-col items-center justify-between h-full w-full py-8 px-6">
               <SlideHeader title="YOU’RE A MULTIPLIER" progressIndex={index} totalSlides={total} />
               <div className="flex flex-col items-center text-center gap-8 flex-1 justify-center -mt-16">
                 ...
                 <div className="space-y-4 max-w-xs">
                   ...
                   >
                     MENTOR TO <span className="text-green-500">{menteeCount}</span><br/>
                   ...
                   >
                     Your knowledge didn’t <br/>stop with you
                   </motion.p>
                 </div>
               </div>
    ...
    {
      id: 'events',
      condition: data.events_2025.organized.concat(data.events_2025.attended, data.events_2025.spokenAt) && data.events_2025.organized.concat(data.events_2025.attended, data.events_2025.spokenAt).length > 0,
      laser: { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "95%", height: "95%", borderRadius: "30px", borderColor: "#60a5fa", rotate: 0, opacity: 0.5 },
      render: (index, total) => {
        ...
        return (
          <div className="flex flex-col items-center h-full w-full relative z-10 overflow-hidden bg-[#0a0a0a]">
             <SlideBackground />
             <div className="relative z-10 flex flex-col items-center justify-between h-full w-full py-8 px-6">
               <SlideHeader title="ON STAGE & BEHIND SCENES" progressIndex={index} totalSlides={total} />
               <div className="flex flex-col items-center w-full max-w-md flex-1 mt-4">
                 ...
                 >
                   You showed up <br/>
                   <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400">In Real Life</span>
                 </motion.h3>
                 <motion.div 
                   ...
                   {displayEvents.map((evt: any, i) => (
                     ...
                     >
                       <div className="relative overflow-hidden bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-0 flex flex-col clip-ticket shadow-lg transition-transform hover:scale-[1.02]">
                          <div className={`absolute top-0 bottom-0 left-0 w-2 ${evt.type === 'speaking' ? 'bg-purple-500' : 'bg-blue-500'}`} />
                          <div className="pl-6 pr-4 py-3 flex items-center justify-between gap-3">
                             <div className="flex-1">
                               <div className="flex items-center gap-2 mb-1">
                                 ...
                                 <span className="text-[10px] text-gray-400 uppercase tracking-widest">{evt.date || '2025'}</span>
                               </div>
                               <h4 className="text-base font-bold text-white leading-tight line-clamp-1">{evt.title}</h4>
                             </div>
                             <div className="opacity-20">
                               {evt.type === 'speaking' ? <Mic className="w-6 h-6" /> : <Calendar className="w-6 h-6" />}
                             </div>
                          </div>
                        ...
                        <div className="bg-white/5 border border-white/10 rounded-lg p-3 flex items-center justify-between backdrop-blur-sm">
                           <div className="flex items-center gap-3">
                              <div className="flex -space-x-2">
                                <div className="w-6 h-6 rounded-full bg-purple-500/50 border border-white/20 flex items-center justify-center text-[8px]">+</div>
                                <div className="w-6 h-6 rounded-full bg-blue-500/50 border border-white/20 flex items-center justify-center text-[8px]">+</div>
                              </div>
                              <span className="text-xs text-gray-300 font-medium">+ And many more...</span>
                           </div>
                           <span className="px-2 py-0.5 bg-white/10 rounded text-[10px] font-bold text-white">{remainingCount} More</span>
                        </div>
                     </motion.div>
                   )}
                 </motion.div>
    ...
    // SLIDE 4: CONTRIBUTOR ROLES
    {
      ...
      laser: { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "95%", height: "95%", borderRadius: "30px", borderColor: "#facc15", rotate: 0, opacity: 0.5 },
      render: (index, total) => {
        const MAX_ROLES = 7;
        ...
        return (
          <div className="flex flex-col items-center h-full w-full relative z-10 overflow-hidden bg-[#0a0a0a]">
             <SlideBackground />
             <div className="relative z-10 flex flex-col items-center justify-between h-full w-full py-8 px-6">
               <SlideHeader title="ROLES & RECOGNITION" progressIndex={index} totalSlides={total} /> 
                 ...
                 >
                    {displayRoles.map((role, idx) => (
                      ...
                      >
                         ...
                         <span className="text-xs font-medium text-gray-200 leading-tight">{role}</span>
                      </motion.div>
                    ))}
                    ...
                    {remainingRoles > 0 && (
                      ...
                      >
                        ...
                        <span className="text-xs font-bold text-yellow-200">And {remainingRoles} more...</span>
                      </motion.div>
                    )}
                 </motion.div>
               </div>
    ...
    // SLIDE 5: TOTAL CONTRIBUTIONS
    {
      ...
      laser: { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "95%", height: "95%", borderRadius: "30px", borderColor: "#fb923c", rotate: 0, opacity: 0.5 },
      render: (index, total) => {
        ...
        return (
          <div className="flex flex-col items-center h-full w-full relative z-10 overflow-hidden bg-[#0a0a0a]">
             <SlideBackground />
             <div className="relative z-10 flex flex-col items-center justify-between h-full w-full py-8 px-6">
               <SlideHeader title="2025 IMPACT" progressIndex={index} totalSlides={total} />
               <div className="flex flex-col items-center text-center gap-6 flex-1 justify-center -mt-10">
                 ...
                 <div className="space-y-4 max-w-xs">
                   ...
                   >
                     YOU KEPT THE <br/>
                     <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-red-500">MOMENTUM</span> ALIVE
                   </motion.h3>
                   <motion.p 
                     initial={{ opacity: 0 }}
                     animate={{ opacity: 1 }}
                   ...
                   >
                     Every commit counts towards<br/> the future.
                   </motion.p>
                 </div>
               </div>
    ...
    // SLIDE 6: DRUPAL CORE
    {
      ...
      laser: { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "95%", height: "95%", borderRadius: "30px", borderColor: "#0678BE", rotate: 0, opacity: 0.5 },
      render: (index, total) => {
        ...
        return (
          <div className="flex flex-col items-center h-full w-full relative z-10 overflow-hidden bg-[#0a0a0a]">
             <SlideBackground />
             <div className="relative z-10 flex flex-col items-center justify-between h-full w-full py-8 px-6">
               <SlideHeader title="CORE CONTRIBUTOR" progressIndex={index} totalSlides={total} />
               <div className="flex flex-col items-center text-center gap-6 flex-1 justify-center -mt-10">
                 <motion.div 
                   initial={{ scale: 0 }}
                   animate={{ scale: 1 }}
                 ...
                 <div className="space-y-4 max-w-xs">
                   ...
                   >
                     YOU BUILT THE <br/>
                     <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-cyan-300">FOUNDATION</span>
                   </motion.h3>
                   <motion.p 
                     initial={{ opacity: 0 }}
                     animate={{ opacity: 1 }}
    ...
    // SLIDE 7: AI LEADERSHIP
    {
      ...
      laser: { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "95%", height: "95%", borderRadius: "30px", borderColor: "#a855f7", rotate: 0, opacity: 0.5 },
      render: (index, total) => {
        ...
        return (
          <div className="flex flex-col items-center h-full w-full relative z-10 overflow-hidden bg-[#0a0a0a]">
             <SlideBackground />
             <div className="relative z-10 flex flex-col items-center justify-between h-full w-full py-8 px-6">
               <SlideHeader title="YOU BROUGHT AI TO DRUPAL" progressIndex={index} totalSlides={total} />
               <div className="flex flex-col items-center text-center gap-6 flex-1 justify-center -mt-10">
                 <motion.div 
                   initial={{ scale: 0 }}
                   animate={{ scale: 1 }}
                 ...
                 <div className="space-y-4 max-w-xs">
                   ...
                   >
                     ...
                     <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400">AI + DRUPAL</span>
                   </motion.h3>
                   <motion.p 
                     initial={{ opacity: 0 }}
                     animate={{ opacity: 1 }}
    ...
    // SLIDE 8: SUMMARY / WRAPPED (FINAL)
    {
      id: 'summary',
      condition: true,
      laser: { top: "50%", left: "50%", x: "-50%", y: "-50%", width: "98%", height: "98%", borderRadius: "20px", borderColor: "#ffffff", rotate: 0, opacity: 0.3 },
      render: (index, total) => {
        ...
        const isQuietYear = !showTotal && !showCoreAI && !showMentorship && !showRoles;

        return (
          <div className="flex flex-col items-center h-full w-full relative z-10 overflow-hidden bg-[#0a0a0a]">
             <SlideBackground />
             <div className="relative z-10 flex flex-col items-center justify-between h-full w-full py-8 px-4 md:px-6">
               <SlideHeader title="2025 YEAR IN REVIEW" progressIndex={index} totalSlides={total} />
                 ...
                 >
                   ...
                   <div className="space-y-4">
                     <h2 className="text-3xl font-bold text-white font-display uppercase tracking-wide">
                       The Quiet Before<br/>
                       <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400">The Storm</span>
                     </h2>
                     <p className="text-gray-300 text-lg">
                       2025 was a quiet year for contributions, but every great feature starts with a pause. 
                       The community awaits your next commit in 2026!
                     </p>
                   </div>
                 </motion.div>
               ) : (
                 ...
                 >
                   ...
                   >
                     <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-cyan-400 shrink-0">
                       <img src={avatarSrc} className="w-full h-full object-cover" alt="User" />
                     </div>
                     <div>
                   ...
                   {showTotal && (
                     <motion.div 
                       variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}
                       className={`${showCoreAI ? 'col-span-1' : 'col-span-2'} bg-gradient-to-br from-orange-900/40 to-black/60 backdrop-blur-md rounded-2xl p-4 border border-orange-500/20 flex flex-col justify-between`}
                     >
                       <GitCommit className="w-6 h-6 text-orange-400 mb-2" />
                       <div>
                         <div className="text-4xl font-black text-transparent bg-clip-text bg-gradient-to-br from-orange-300 to-white">{data.total_issues_count}</div>
                         <div className="text-xs text-orange-200/70 font-medium mt-1">TOTAL CONTRIBS</div>
                       </div>
                     </motion.div>
                   )}
                   ...
                   {/* Card 3: Core & AI */}
                   {showCoreAI && (
                     <div className={`flex flex-col gap-3 ${showTotal ? 'col-span-1' : 'col-span-2'}`}>
                        {showCore && (
                          <motion.div 
                            variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}
                            className={`flex-1 bg-gradient-to-br from-blue-900/40 to-black/60 backdrop-blur-md rounded-2xl p-3 border border-blue-500/20 flex items-center justify-between ${!showAI ? 'h-full' : ''}`}
                          >
                            <div className="flex flex-col">
                              <span className="text-2xl font-bold text-blue-200">{data.total_issues_for_drupal_count}</span>
                              <span className="text-[10px] text-blue-300/70 uppercase">Core</span>
                            </div>
                        ...
                        {showAI && (
                          <motion.div 
                            variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }}
                            className={`flex-1 bg-gradient-to-br from-purple-900/40 to-black/60 backdrop-blur-md rounded-2xl p-3 border border-purple-500/20 flex items-center justify-between ${!showCore ? 'h-full' : ''}`}
                          >
                            <div className="flex flex-col">
                              <span className="text-2xl font-bold text-purple-200">{data.total_issues_for_ai_count}</span>
                              <span className="text-[10px] text-purple-300/70 uppercase">AI</span>
                            </div>
                     ...
                     >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-green-400 font-mono">{data.mentee_count} Contributors</div>
                          </div>
                        </div>
                   ...
                   {showRoles && (
                     ...
                     >
                        ...
                        <div className="flex flex-wrap gap-2">
                          {topRoles.map((r, i) => (
                            <span key={i} className="text-[10px] px-2 py-1 bg-white/10 rounded-full text-gray-300 border border-white/5 truncate max-w-[150px]">{r}</span>
                          ))}
                          {hasMoreRoles && (
                            <span className="text-[10px] px-2 py-1 bg-yellow-500/20 text-yellow-200 rounded-full border border-yellow-500/30">+{roles.length - 3} more</span>
                          )}
                        </div>
                     </motion.div>
                   )}
  ...
  // Share Handler
  const handleShare = async (e: React.MouseEvent, platform: 'linkedin' | 'x') => {
    e.stopPropagation();
    ...
    try {
      ...
      const canvas = await html2canvas(storyRef.current, {
        ...
        onclone: (clonedDoc) => {
          ...
          const gradientTexts = clonedDoc.querySelectorAll('.bg-clip-text') as NodeListOf<HTMLElement>;
          gradientTexts.forEach((el) => {
            ...
            if (el.className.includes('from-orange')) el.style.color = '#fb923c';
            else if (el.className.includes('from-blue')) el.style.color = '#60a5fa';
            else if (el.className.includes('from-purple')) el.style.color = '#a855f7';
            else if (el.className.includes('from-green')) el.style.color = '#4ade80';
            else if (el.className.includes('from-cyan')) el.style.color = '#22d3ee';
            else if (el.className.includes('from-indigo')) el.style.color = '#818cf8';
          });

          // Explicitly hide no-capture elements (double safety)
          const ignored = clonedDoc.querySelectorAll('.no-capture') as NodeListOf<HTMLElement>;
      ...
      // 3. Open Social Media
      const currentUrl = window.location.href; // Capture current slug URL
      const text = `Check out my Drupal 2025 Year in Review! 💧✨ #Drupal #OpenSource #Drupal2025 ${currentUrl}`;
      let shareUrl = '';
    ...
    >
      ...
      {currentSlideDef.id === 'intro' && (
        <div className="absolute top-0 left-0 w-full h-full pointer-events-none opacity-40 mix-blend-screen">
          <div className={`absolute top-[-20%] left-[-20%] bg-purple-900 w-[80%] h-[80%] rounded-full blur-[120px] animate-pulse`} />
          <div className={`absolute bottom-[-20%] right-[-20%] bg-cyan-900 w-[80%] h-[80%] rounded-full blur-[120px] animate-pulse`} style={{ animationDelay: '2s' }} />
        </div>
      )}
      ...
      <AnimatePresence>
        {!isPlaying && (
          <motion.div 
            ...
            onClick={(e) => { e.stopPropagation(); setIsPlaying(true); }} // Clicking background resumes
          >
            <div className="flex flex-col items-center gap-6 p-8 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
              <h2 className="text-3xl font-bold text-white tracking-widest uppercase mb-4">Paused</h2>
              ...
              >
                {isSharing === 'linkedin' ? <Loader2 className="w-6 h-6 animate-spin" /> : <Linkedin className="w-6 h-6" />}
                Share on LinkedIn
              </button>
              ...
              >
                {isSharing === 'x' ? <Loader2 className="w-6 h-6 animate-spin" /> : <Twitter className="w-6 h-6" />}
                Share on X
              </button>
      ...
      <div className="absolute top-0 left-0 w-1/4 h-full z-20" onClick={(e) => { e.stopPropagation(); prevSlide(); }}></div>
      <div className="absolute top-0 right-0 w-1/4 h-full z-20" onClick={(e) => { e.stopPropagation(); nextSlide(); }}></div>
      ...
      >
         <div className="absolute -top-1 -left-1 w-3 h-3 border-t-2 border-l-2" style={{ borderColor: 'white' }} />
         <div className="absolute -top-1 -right-1 w-3 h-3 border-t-2 border-r-2" style={{ borderColor: 'white' }} />
         <div className="absolute -bottom-1 -left-1 w-3 h-3 border-b-2 border-l-2" style={{ borderColor: 'white' }} />
         <div className="absolute -bottom-1 -right-1 w-3 h-3 border-b-2 border-r-2" style={{ borderColor: 'white' }} />
