...
const UsernameForm: React.FC<UsernameFormProps> = ({
  ...
  return (
    <div
      className={`min-h-dvh flex flex-col items-center justify-center p-6 ${themeStyles.bg}`}
    >
      ...
      >
        <div className="text-center mb-10">
          ...
          <h1
            className={`display-font text-4xl font-bold mb-4 ${themeStyles.text}`}
          >
        ...
        <div
          className={`p-8 rounded-3xl shadow-xl border ${themeStyles.card} ${themeStyles.border} transition-all duration-300 min-h-[420px] flex flex-col justify-center`}
        >
          {!loading ? (
            <>
              ...
              <div className="mt-8 pt-6 border-t border-gray-100 dark:border-gray-800 text-center">
                <p
                  className={`py-2 text-xs font-medium ${themeStyles.textMuted} mx-auto`}
                >
                  Your Drupal username appears in parentheses on your profile.
                  <br />
                  Example: Dries Buytaert (dries) — <strong className="text-white">dries</strong> is
                  the username.
                </p>

                <p
                  className={`text-xs ${themeStyles.textMuted} mt-2 max-w-xs mx-auto`}
                >
                  Disclaimer: The data is sourced from the Drupal.org APIs and
                  publicly available Drupal profile information.
                </p>
              </div>
            </>
          ) : (
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <p className={`text-sm ${themeStyles.textMuted} max-w-[280px]`}>
                We're digging through your contributions. Play a quick game of
              ...
              />
              <p
                className={`text-xs ${themeStyles.textMuted} max-w-[280px] italic`}
              >
                Wait times may be slightly longer depending on rate limits and
